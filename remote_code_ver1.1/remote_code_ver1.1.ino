...
//LiquidCrystal lcd(12,11,5,4,3,2 );        /* Pins to control LCD display */
int adc_key_val[5] ={50, 200, 400, 600, 800 };  /* Analog values from keypad */
#define SPEAKER_OUT  6                        /* Pin to connect speaker */
...
void setup(){
    ...
    characters();
    pinMode(13, OUTPUT);
    for(int i=30;i<40;i++)pinMode(i, INPUT);     
...
void loop(){
    ...
    Serial1.write('S');
    delay(50);
    Serial1.write(lastM[0]);
    delay(50); 
    Serial1.write(lastM[1]);
    delay(50);
    Serial1.write(lastM[2]);
    delay(50);
    Serial1.write(lastM[3]);
    delay(50);
    //Serial1.write('t');
    delay(50);
    ...
    Serial2.write('S');
    delay(50);
    Serial2.write(lastM[0]);
    delay(50);
    Serial2.write(lastM[1]);
    delay(50);
    Serial2.write(lastM[2]);
    delay(50);
    Serial2.write(lastM[3]);
    delay(50);
...
void takeMove(){
    ...
    for(;;){
        ...
        switch(k){
          case 0:
            if(select==0){x1=0; oneNote(NOTE_C8, 23);}
            else if(select==1){y1=0; oneNote(NOTE_C8, 23);}
            else if(select==2){x2=0; oneNote(NOTE_C8, 23);}
            else if(select==3){y2=0; oneNote(NOTE_C8, 23);}
            break;
          case 1:
            if(select==0){x1=1; oneNote(NOTE_C8, 23);}
            else if(select==1){y1=1; oneNote(NOTE_C8, 23);}
            else if(select==2){x2=1; oneNote(NOTE_C8, 23);}
            else if(select==3){y2=1; oneNote(NOTE_C8, 23);}
            break;
          case 2:
            if(select==0){x1=2; oneNote(NOTE_C8, 23);}
            else if(select==1){y1=2; oneNote(NOTE_C8, 23);}
            else if(select==2){x2=2; oneNote(NOTE_C8, 23);}
            else if(select==3){y2=2; oneNote(NOTE_C8, 23);}
            break;
          case 3:
            if(select==0){x1=3; oneNote(NOTE_C8, 23);}
            else if(select==1){y1=3; oneNote(NOTE_C8, 23);}
            else if(select==2){x2=3; oneNote(NOTE_C8, 23);}
            else if(select==3){y2=3; oneNote(NOTE_C8, 23);}
            break;
          case 4:
            if(select==0){x1=4; oneNote(NOTE_C8, 23);}
            else if(select==1){y1=4; oneNote(NOTE_C8, 23);}
            else if(select==2){x2=4; oneNote(NOTE_C8, 23);}
            else if(select==3){y2=4; oneNote(NOTE_C8, 23);}
            break;
          case 5:
            if(select==0){x1=5; oneNote(NOTE_C8, 23);}
            else if(select==1){y1=5; oneNote(NOTE_C8, 23);}
            else if(select==2){x2=5; oneNote(NOTE_C8, 23);}
            else if(select==3){y2=5; oneNote(NOTE_C8, 23);}
            break;
          case 6:
            if(select==0){x1=6; oneNote(NOTE_C8, 23);}
            else if(select==1){y1=6; oneNote(NOTE_C8, 23);}
            else if(select==2){x2=6; oneNote(NOTE_C8, 23);}
            else if(select==3){y2=6; oneNote(NOTE_C8, 23);}
            break;
          case 7:
            if(select==0){x1=7; oneNote(NOTE_C8, 23);}
            else if(select==1){y1=7; oneNote(NOTE_C8, 23);}
            else if(select==2){x2=7; oneNote(NOTE_C8, 23);}
            else if(select==3){y2=7; oneNote(NOTE_C8, 23);}
            break;
...
int getKey(){
    ...
    if(digitalRead(30)==HIGH)k=0;
    else if(digitalRead(31)==HIGH)k=1;
    else if(digitalRead(32)==HIGH)k=2;
    else if(digitalRead(33)==HIGH)k=3;
    else if(digitalRead(34)==HIGH)k=4;
    else if(digitalRead(35)==HIGH)k=5;
    else if(digitalRead(36)==HIGH)k=6;
    else if(digitalRead(37)==HIGH)k=7;
    else if(digitalRead(38)==HIGH)k=8;
    else if(digitalRead(39)==HIGH)k=9;    
...
void displayBoard(int ind){
    ...
    lcd.setCursor(2, 2);
    lcd.print(board[ind+2]);
    lcd.setCursor(2, 3);
    lcd.print(board[ind+3]);//초기 4줄 출력
...
/*******************************
Calling this function ARDUINO
plays using Serial Monitor
with "newline" in all inputs.
...
void playSerialMode(){
    ...
    SP("*-> N=");SP(N,HEX);SP(", I=");SP(I,HEX);SP(", T=");SP(T,HEX);SP(", Q=");SP(Q,HEX);SP(", O=");SP(O,HEX);SP(", K=");SP(K,HEX);SP(", R=");SP(R,HEX);SP(", k=");SP(k,HEX);SP(", Z=");SP(Z,HEX);Serial.println(">");
    for(;;){                                      /* play loop          */
        p=c;
        W((*p++=mygetchar())>10);                 /* read input line    */
        K=I;                                      /* invalid move       */
        if(*c-10){
            K=*c-16*c[1]+799,L=c[2]-16*c[3]+799;  /* parse entered move */
        }else{
        ...
        c[4]=0;
        Serial.print("  ### Human move: ");    myputs(c);
        N=0;
        T=0x3F;                                   /* T=Computer Play strength */        
        ...
        bkp();                                    /* Save the board just in case */
        r = D(-I,I,Q,O,1,3);
        Serial.print("  ### Post-Human move: ");    myputs(c);
        SP("A-> N=");SP(N,HEX);SP(", I=");SP(I,HEX);SP(", T=");SP(T,HEX);SP(", Q=");SP(Q,HEX);SP(", O=");SP(O,HEX);SP(", K=");SP(K,HEX);SP(", R=");SP(R,HEX);SP(", k=");SP(k,HEX);SP(", Z=");SP(Z,HEX);SP(", r=");Serial.println(r);
        if( !(r>-I+1) ) break;
        if(k == 0x10){     /*  Check the human move is valid*/
        ...
        searchDataBase();
        if(c[0]){                                 /* Movement found */
            Serial.print("  ### DataBase move: ");  myputs(c);
            K=*c-16*c[1]+799,L=c[2]-16*c[3]+799;  /* parse move found */
            N=0;
            T=0x3F;                               /* T=Computer Play strength */

            r = D(-I,I,Q,O,1,3);                  /* Check & do*/
            SP("DB-> N=");SP(N,HEX);SP(", I=");SP(I,HEX);SP(", T=");SP(T,HEX);SP(", Q=");SP(Q,HEX);SP(", O=");SP(O,HEX);SP(", K=");SP(K,HEX);SP(", R=");SP(R,HEX);SP(", k=");SP(k,HEX);SP(", Z=");SP(Z,HEX);SP(", r=");Serial.println(r);
            serialBoard();
            if( !(r>-I+1) ) break;
            if(k == 0x08){
                Serial.println("*** ERROR IN DATABASE ***");
                break;
            }
            continue;
        }        
        ...
        T=0x3F;                                   /* T=Computer Play strength */
        r = D(-I,I,Q,O,1,3);                      /* Think & do*/
        Serial.print("  ### Arduino move: "); myputs(c);
        SP("B-> N=");SP(N,HEX);SP(", I=");SP(I,HEX);SP(", T=");SP(T,HEX);SP(", Q=");SP(Q,HEX);SP(", O=");SP(O,HEX);SP(", K=");SP(K,HEX);SP(", R=");SP(R,HEX);SP(", k=");SP(k,HEX);SP(", Z=");SP(Z,HEX);SP(", r=");Serial.println(r);
        if( !(r>-I+1) ) break;
        
        /* Some times the algorithm no dot execute the move 
            1.- b1c3  c7c5
            2.- f2f4?
        */
        if(k == 0x08){
            serialBoard();
            Serial.println("*** ERROR 3 -- Please reset *** ");
            break;
        }
        
        serialBoard();
        
        /* Execute a invalid move to check score again */
        strcpy(c, "a1a1");
        r = D(-I,I,Q,O,1,3);
        Serial.print("  ### a1a1 move: "); myputs(c);
        SP("C-> N=");SP(N,HEX);SP(", I=");SP(I,HEX);SP(", T=");SP(T,HEX);SP(", Q=");SP(Q,HEX);SP(", O=");SP(O,HEX);SP(", K=");SP(K,HEX);SP(", R=");SP(R,HEX);SP(", k=");SP(k,HEX);SP(", Z=");SP(Z,HEX);SP(", r=");Serial.println(r);        
        if( !(r>-I+1) ) break;        
        
    }

    Serial.print("\n\n *** End Game --- Please RESET *** ");
}
